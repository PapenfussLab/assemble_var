...
def getOptions():
	...
	parser.add_option('', '--velveth',dest='velveth',help='path to velveth',default='velveth')
	parser.add_option('', '--velvetg',dest='velvetg',help='path to velvetg',default='velvetg')
	parser.add_option('', '--oases',dest='oases',help='path to oases',default='oases')
	parser.add_option('', '--single',dest='singleOnly', help='only run single k assemblies',action='store_true',default=False)
...
def singleKAssemblies(options):
	p = subprocess.Popen([options.velveth, options.directoryRoot, '%i,%i,%i' % (options.kmin, options.kmax+options.kstep, options.kstep)] + options.data.split(), stdout=subprocess.PIPE)
	output = p.communicate()
	assert p.returncode == 0, output[0] + "Hash failed\n"
	for k in range(options.kmin, options.kmax+options.kstep, options.kstep):
	    p = subprocess.Popen([options.velvetg,'%s_%i' % (options.directoryRoot, k), '-read_trkg', 'yes'], stdout=subprocess.PIPE)
	    output = p.communicate()
	    assert p.returncode == 0, "Velvetg failed at k = %i\n%s" % (k, output[0])
	    p = subprocess.Popen([options.oases,'%s_%i' % (options.directoryRoot, k)] + options.oasesOptions.split(), stdout=subprocess.PIPE)
	    output = p.communicate()
...
def mergeAssemblies(options):
	...
	p = subprocess.Popen([options.velveth,'%sMerged' % options.directoryRoot, str(options.kmerge), '-long'] + files, stdout=subprocess.PIPE)
	output = p.communicate()
	assert p.returncode == 0, output[0] + "Velveth failed at merge\n"
	p = subprocess.Popen([options.velvetg,'%sMerged' % options.directoryRoot,'-conserveLong','yes','-read_trkg','yes'], stdout=subprocess.PIPE)
	output = p.communicate()
	assert p.returncode == 0, output[0] + "Velvetg failed at merge\n"
	p = subprocess.Popen([options.oases,'%sMerged' % options.directoryRoot,'-merge','yes'], stdout=subprocess.PIPE)
	output = p.communicate()
...
##########################################
def checkVelvet(options):
	try:
	    p = subprocess.Popen([options.velveth], stdout=subprocess.PIPE)
	except OSError:
...
def checkOases(options):
	try:
	    p = subprocess.Popen([options.oases], stdout=subprocess.PIPE)
	except OSError:
